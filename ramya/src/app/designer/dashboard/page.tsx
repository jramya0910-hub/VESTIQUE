...
import { LayoutDashboard, Package, ShoppingBag, Upload, TrendingUp, Pencil, Trash2, X, Save } from 'lucide-react'
import { useAuth } from '@/lib/auth-context'
import { supabase, Product, authedFetch } from '@/lib/supabase'
...
export default function DesignerDashboardPage() {
  ...
  async function handleDelete(productId: string, productName: string) {
    ...
    const res = await authedFetch(`/api/products?id=${productId}`, { method: 'DELETE' })
    if (res.ok) {
  ...
  async function handleSaveEdit() {
    ...
    const res = await authedFetch('/api/products', {
      ...
      body: JSON.stringify({ id: editingProduct.id, ...editForm, price: Number(editForm.price) }),
    })
    const data = await res.json()
    if (res.ok) {
  ...
  const CATEGORIES_EDIT = ['Sarees', 'Kurtas', 'Lehengas', 'Gowns', 'Dresses', 'Tops', 'Co-ords', 'Other']
  ...
  return (
    <div>
      ...
      {/* Edit Modal */}
      {editingProduct && (
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4" onClick={e => e.target === e.currentTarget && setEditingProduct(null)}>
          <div className="bg-white rounded-2xl p-6 w-full max-w-lg shadow-2xl">
            <div className="flex items-center justify-between mb-5">
              <h3 className="text-lg font-bold text-gray-900">Edit Product</h3>
              <button onClick={() => setEditingProduct(null)}><X size={20} className="text-gray-400 hover:text-gray-700" /></button>
            </div>
            <div className="space-y-4">
              <div><label className="label">Name</label><input className="input mt-1" value={editForm.name} onChange={e => setEditForm(f => ({ ...f, name: e.target.value }))} /></div>
              <div><label className="label">Price (₹)</label><input className="input mt-1" type="number" value={editForm.price} onChange={e => setEditForm(f => ({ ...f, price: e.target.value }))} /></div>
              <div>
                <label className="label">Category</label>
                <select className="input mt-1" value={editForm.category} onChange={e => setEditForm(f => ({ ...f, category: e.target.value }))}>
                  {CATEGORIES_EDIT.map(c => <option key={c} value={c}>{c}</option>)}
                </select>
              </div>
              <div><label className="label">Description</label><textarea className="input mt-1 resize-none" rows={3} value={editForm.description} onChange={e => setEditForm(f => ({ ...f, description: e.target.value }))} /></div>
              <div><label className="label">Image URL</label><input className="input mt-1" value={editForm.image_url} onChange={e => setEditForm(f => ({ ...f, image_url: e.target.value }))} /></div>
            </div>
            <div className="flex gap-3 mt-6">
              <button onClick={handleSaveEdit} disabled={savingEdit} className="btn-primary flex items-center gap-2 text-sm flex-1 justify-center">
                <Save size={14} /> {savingEdit ? 'Saving…' : 'Save Changes'}
              </button>
              <button onClick={() => setEditingProduct(null)} className="btn-secondary text-sm">Cancel</button>
            </div>
          </div>
        </div>
      )}
            ...
            <div className="space-y-3">
              {products.map(p => (
                <div key={p.id} className="flex items-center gap-3 bg-white border border-gray-200 rounded-xl p-4">
                  ...
                  <p className="font-bold text-violet-700 text-sm flex-shrink-0 mr-2">
